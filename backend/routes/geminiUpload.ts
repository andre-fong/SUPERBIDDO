...
import { preserveImage } from './images';
    ...
    return `DONT PUT \`\`\`json at the start. Also never give an empty string for any of these in each json field.
        ...
        Ex.
            ...
            - quality: The condition of the card this only consist of
                ${qualityList.join(", ")} PLEASE MAKE SURE TO ALWAYS GIVE A REPONSE FOR THIS GIVE YOUR BEST GUESS
...
router.get("/:imageUrl", async (req: express.Request, res: express.Response) => {
    ...
    res.json({ response: JSON.stringify(JSON.parse(responseText)) });
})